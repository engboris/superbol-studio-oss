...
(* Copybooks are looked up in absolute directories, so that locations in a
   copybook can be matched with the document opened in the editor. *)
let copybook_lookup_config_for ~uri project =
  let lookup_config =
    Superbol_project.copybook_lookup_config_for ~filename:(Lsp.Uri.to_path uri)
      project
  in
  let rootdir = string_of_rootdir @@ rootdir project in
  { lookup_config with
    lookup_path = List.map (Lsp_utils.absolute_path ~rootdir)
        lookup_config.lookup_path }
