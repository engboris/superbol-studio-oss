...
class virtual ['a] folder = object
  ...
  method fold_stop_returning      : (stop_returning          , 'a) fold = default
...
let fold_stop_run (v: _ #folder) =
  handle v#fold_stop_run
    ~continue:begin fun { status_kind; status_value } x -> x
      ...
      >> fold_ident_or_literal v status_value
    end
...
let fold_stop_returning (v: _ #folder) =
  handle v#fold_stop_returning
...
let fold_stop_with_status (v: _ #folder) =
  handle v#fold_stop_with_status
    ~continue:begin fun { status_kind = _status_kind; status_value } x -> x
      >> fold_ident_or_literal v status_value
    end
...
let fold_stop' (v: _ #folder) =
  handle' v#fold_stop' v
    ~fold:begin fun v -> function
      | StopSimple | StopError -> Fun.id
      | StopRunReturning o -> fold_option ~fold:fold_stop_returning v o
      | StopRunWithStatus s -> fold_stop_with_status v s
      | StopWithArg a -> fold_name_or_literal v a
    end
